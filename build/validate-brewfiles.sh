...
# through Homebrew's evaluator, so a PR-controlled Brewfile would get code
# execution in CI. This script only ever greps the files — entries are passed
# to `brew info` as data — and rejects anything that is not a quoted literal.
#
# Adapted from projectbluefin/finpilot build/validate-brewfiles.sh, which in
# turn ports projectbluefin/common scripts/validate-brewfiles.sh with one
...
#
# This syncs declared taps but never installs formulae or casks.
...
main() (
    ...
    # trailing code or string interpolation (#{...}, #$var, #@ivar) fails closed
    # here. `#` is therefore excluded from the quoted name in both quote styles.
    ...
    # Reject malformed declarations rather than silently skipping them. Only a
    # quoted literal name followed by optional whitespace/comment is accepted:
    # any Ruby argument after a comma (system(...), backticks, args:, ...) would
    # otherwise be copied into the image and evaluated at runtime by `brew
